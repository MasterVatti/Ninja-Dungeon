...
namespace Homebrew
{
    ...
    public class EditorOverride : Editor
    {
        ...
        private void Awake()
        {
            ...
            var uiTex_in_on =
                Resources.Load<Texture2D>("IN foldout focus on-5718");
        ...
        void OnEnable()
        {
            ...
            objectFields = target.GetType().GetFields(BindingFlags.Public |
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (!initialized)
            {
                ...
                {
                    var fold =
                        Attribute.GetCustomAttribute(objectFields[i],
                            typeof(FoldoutAttribute)) as FoldoutAttribute;
            ...
            using (new EditorGUI.DisabledScope("m_Script" ==
                                               props[0].propertyPath))
            {
            ...
            foreach (var pair in cache)
            {
                ...
                EditorGUI.DrawRect(
                    new Rect(rect.x - 1, rect.y - 1, rect.width + 1,
                        rect.height + 1),
                    colors.col0);

                EditorGUI.DrawRect(
                    new Rect(rect.x - 1, rect.y - 1, rect.width + 1,
                        rect.height + 1), colors.col1);


                pair.Value.expanded = EditorGUILayout.Foldout(
                    pair.Value.expanded, pair.Value.atr.name, true,
                    ...
                    EditorGUILayout.Space();
                    {
                        for (int i = 0; i < pair.Value.props.Count; i++)
                        {
                            ...
                            EditorGUILayout.PropertyField(pair.Value.props[i],
                                new GUIContent(pair.Value.props[i].name
                                    .FirstLetterToUpperCase()), true);
