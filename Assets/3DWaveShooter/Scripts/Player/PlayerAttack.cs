...
{
    public LayerMask
        raycastProjectileLayerMask; //Layers that will be detected when shooting a raycast.
    public GameObject
        trailPrefab; //Prefab of trail to draw for raycast projectiles.
    ...
    private int
        curBurstProjectilesShot; //Num of bullets shot in current burst.
        ...
        if (Time.time - lastWeaponAttackTime > weapon.shootFrequency)
        {
            ...
            if (weapon.curAmmoInClip > 0)
            {
                ...
                if (inputHeldDown)
                {
                    ...
                    else if (weapon.typeOfWeapon == WeaponType.Burst &&
                             !burstFiring)
                        return;
                }
    ...
    void Shoot()
    {
            ...
            ProjectileType.Projectile)
        {
            ...
            {
                float angleOffset =
                    proj.multipleProjectiles.projectileSpreadAngle / 2;
                float segment =
                    Mathf.Abs(proj.multipleProjectiles.projectileSpreadAngle /
                              (proj.multipleProjectiles.projectileCount - 1));

                for (int i = 0;
                    i < proj.multipleProjectiles.projectileCount;
                    ++i)
                {
                    if (i == 0)
    ...
    void ShootRaycast(float angle)
    {
        ...
        if (Physics.Raycast(ray, out hit, weapon.projectile.raycastLength,
            raycastProjectileLayerMask))
        {
            if (hit.collider.tag == "Enemy")
            {
                hit.collider.GetComponent<EnemyShooter>()
                    .TakeDamage(weapon.projectile.damage, hit.point,
                        hit.normal);
                hit.collider.GetComponent<Rigidbody>().AddForce(
                    -hit.normal * weapon.enemyKnockback, ForceMode.Impulse);

                if (weapon.projectile.applyEffects)
                {
                    for (int i = 0;
                        i < weapon.projectile.effectsToApply.Length;
                        ++i)
                    {
                        new Effect(weapon.projectile.effectsToApply[i],
            ...
            else if (hit.collider.tag == "Damageable")
            {
                hit.collider.GetComponent<Damageable>()
                    .TakeDamage(weapon.projectile.damage);
            }

            //Impact particle.
            GameObject impactParticle = Pool.Spawn(
                ParticleManager.inst.environmentImpact, hit.point,
                Quaternion.identity);
        ...
        if (weapon.projectile.drawTrail)
        {
            ...
            GameObject trail = Pool.Spawn(trailPrefab,
                weaponPos.transform.position, weaponPos.transform.rotation);
