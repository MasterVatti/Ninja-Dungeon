...
public class Player3DWaveShooter : MonoBehaviour
{
    ...
    public WeaponScriptableObject
        startingWeapon; //Weapon the player starts the game with.
    ...
    public List<Weapon>
        weapons = new List<Weapon>(); //Player's inventory of weapons.
    private GameObject
        curWeaponObject; //Player's current weapon game object visual.
    ...
    [Header("Components")]
    public GameObject
        weaponPos; //Position the player will hold their weapon at.
    ...
    public void TakeDamage(int damage)
    {
        ...
        AudioManager.inst.Play(audioSource,
            AudioManager.inst.playerImpactSFX[
                Random.Range(0, AudioManager.inst.playerImpactSFX.Length)]);
    ...
    public void GiveWeapon(Weapon weapon)
    {
        ...
        if (!weapon.onPlayerVisual)
        {
            ...
            weapon.onPlayerVisual = Instantiate(weapon.visualPrefab,
                weaponPos.transform.position + weapon.offsets.positionOffset,
                weaponPos.transform.rotation, weaponPos.transform);
            weapon.onPlayerVisual.transform.localEulerAngles +=
                weapon.offsets.rotationOffset;
    ...
    public void DropWeapon(Weapon weaponToDrop)
    {
        ...
        if (weapons.Find(x => x.displayName == weaponToDrop.displayName) ==
            null)
            return;

        //Spawn the weapon pickup.
        GameObject droppedWeapon = Instantiate(weaponToDrop.droppedPickup,
            transform.position, Quaternion.identity, null);
        droppedWeapon.transform.position = new Vector3(transform.position.x,
