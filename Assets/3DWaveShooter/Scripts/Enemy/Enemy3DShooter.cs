...
public class Enemy3DShooter : MonoBehaviour
{
    [Header("Stats")]
    public int curHp;
    ...
    public int
        moneyGivenOnDeath; //How much money the player gets for killing this enemy.
    ...
    private bool
        impactStunned; //Is the enemy currently impact stunned (reduced speed when damaged)?

    private bool
        hasDamageOverTimeEffect; //Does the enemy have a damage over time effect already on it?
    ...
    public void TakeDamage(int damage)
    {
        ...
        //Particle effect.
        Pool.Spawn(ParticleManager.inst.bloodImpact, transform.position,
            Quaternion.identity);
    }
    ...
    public void TakeDamage(int damage, Vector3 impactPos, Vector3 forward)
    {
        ...
        //Particle effect.
        Pool.Spawn(ParticleManager.inst.bloodImpact, impactPos,
            Quaternion.identity).transform.forward = forward;

        //Sound effect.
        AudioManager.inst.Play(audioSource,
            AudioManager.inst.enemyImpactSFX[
                Random.Range(0, AudioManager.inst.enemyImpactSFX.Length)]);
    }
    ...
    public void Die()
    {
        ...
        //Sound effect.
        AudioManager.inst.Play(audioSource,
            AudioManager.inst.enemyDeathSFX[
                Random.Range(0, AudioManager.inst.enemyDeathSFX.Length)]);
    }
        ...
        GameObject visual)
    {
        if (!hasDamageOverTimeEffect)
            StartCoroutine(
                DamageOverTimeTimer(damage, rate, duration, visual));
    }

    //Same as above.
    IEnumerator DamageOverTimeTimer(int damage, float rate, float duration,
        GameObject visual)
    {
        ...
        if (visual)
            visualObj = Pool.Spawn(visual, transform.position,
