...
public class Projectile3D : MonoBehaviour
{
    ...
    public EffectScriptableObject[]
        effects; //Effects applied to hit entities.

    private bool
        valuesSet; //Used for knowing if the projectile has its value set.
    ...
    void OnTriggerEnter(Collider col)
    {
        ...
        if (col.tag == "Enemy")
        {
            col.GetComponent<Enemy3DShooter>().TakeDamage(damage, transform.position,
