...
public class Pool : MonoBehaviour
{
    ...
    public void SpawnSetOfPoolObjects(GameObject objectToSpawn, int amount)
    {
        ...
        PoolContainer container =
            Pool.inst.objects.Find(
                x => x.objPrefab.name == objectToSpawn.name);
    ...
    public static GameObject Spawn(GameObject objectToSpawn, Vector3 position,
        Quaternion rotation)
    {
    ...
    {
        return Pool.inst.SpawnObject(objectToSpawn, position, rotation,
            parent);
    }
        ...
        Quaternion rotation, Transform parent)
    {
        ...
        {
            PoolContainer newContainer =
                Pool.inst.CreateNewPoolContainer(objectToSpawn);
            GameObject obj =
                Pool.inst.InstantiateNewObjectToPool(newContainer);
    ...
    {
        Pool.inst.StartCoroutine(
            Pool.inst.DestroyDelay(objectToDestroy, time));
    }
    ...
    public static void Destroy(GameObject objectToDestroy)
    {
        ...
        PoolContainer container =
            Pool.inst.objects.Find(x =>
                x.objPrefab.name == objectToDestroy.name);
