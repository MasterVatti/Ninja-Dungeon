...
{
    public GameObject[]
        spawnPoints; //Array of all the spawn points in the level.
    ...
    public int
        nextWaveIndex; //Index for waves[], which indicates the next wave.
    public List<int>
        enemiesToSpawn =
            new List<int>(); //List of enemies to spawn where the int is the index for the different enemies in a WaveData class.

    private float
        lastTimeSpawnEnemy; //Time at which the last enemy was spawned.
    private int
        curEnemyToSpawnIndex; //Index of the current enemy to spawn, relates to enemiesToSpawn[].
    private int
        curSpawnPointIndex; //Index of the current spawn point, relates to spawnPoints[].
    private int bossSpawnIndex; //CurEnemyToSpawnIndex where the boss spawns.

    public int
        remainingEnemies; //Amount of enemies that remain in the current wave.

    public bool
        spawnEnemies; //Are we spawning enemies? Cuts off once all the enemies of a wave have spawned.
    ...
    public void SetNewWave()
    {
        ...
        if (curEnemies.Count > 0)
        {
            for (int x = 0; x < curEnemies.Count; ++x)
                curEnemies[x].GetComponent<Enemy3DShooter>().SinkAndDestroy();
        }
        ...
        {
            bossSpawnIndex = enemiesToSpawn.Count *
                             Mathf.CeilToInt(0.01f *
                                             (float) curWave.boss
                                                 .wavePercentToSpawnBoss);
    ...
    void Update()
    {
        ...
        if (spawnEnemies && spawnPoints.Length > 0)
        {
            ...
            if (Time.time - lastTimeSpawnEnemy > curWave.enemySpawnRate)
            {
                ...
                SpawnEnemy(
                    curWave.enemyTypes[enemiesToSpawn[curEnemyToSpawnIndex]]
                        .enemy,
                    spawnPoints[curSpawnPointIndex].transform.position +
                    new Vector3(Random.Range(-1.0f, 1.0f), 0,
                ...
                if (curWave.boss.spawnBoss)
                {
                    ...
                    {
                        SpawnEnemy(curWave.boss.boss,
                            spawnPoints[curSpawnPointIndex].transform
                                .position +
                            new Vector3(Random.Range(-1.0f, 1.0f), 0,
                                Random.Range(-1.0f, 1.0f)));
                        remainingEnemies++;
        ...
        if (GameManager.inst.waveInProgress)
        {
            if (remainingEnemies == 0 &&
                GameManager.inst.curWaveTime >
                GameManager.inst.waveCountdownTime + 2)
            {
