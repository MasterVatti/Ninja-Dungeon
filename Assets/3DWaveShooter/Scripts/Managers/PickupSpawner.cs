...
public class PickupSpawner : MonoBehaviour
{
    ...
    public GameObject[]
        pickupSpawnPoints; //Positions to spawn the pickups at.

    public float averageSpawnRate; //Average amount of time between spawning.
    public float
        spawnRateRandomness; //Random range applied to average spawn rate.
    ...
    void SpawnPickup()
    {
        ...
        Vector3 offset = new Vector3(Random.Range(-1.0f, 1.0f), 0,
            Random.Range(-1.0f, 1.0f));

        GameObject pickupObj = Instantiate(pickups[pickup],
            pickupSpawnPoints[spawnPoint].transform.position + offset,
            Quaternion.identity);
    ...
    float GetNextPickupSpawnTime()
    {
        return Time.time +
               averageSpawnRate +
               Random.Range(-spawnRateRandomness / 2,
                   spawnRateRandomness / 2);
    }
}
