...
public class ScreenFade : MonoBehaviour
{
    ...
    float GetTargetDurationModifier(float colourComponent,
        float fadeImageColorComponent)
    {
        return Mathf.Clamp(
            Mathf.Abs(colourComponent - fadeImageColorComponent) /
            targetDuration, 0.0f, 99.0f);
    }
    ...
    void Update()
    {
        if (fadeImage.color != targetColor)
        {
            fadeImage.color = new Color(
                Mathf.MoveTowards(fadeImage.color.r, targetColor.r,
                    targetDurMod.x * Time.deltaTime),
                Mathf.MoveTowards(fadeImage.color.g, targetColor.g,
                    targetDurMod.y * Time.deltaTime),
                Mathf.MoveTowards(fadeImage.color.b, targetColor.b,
                    targetDurMod.z * Time.deltaTime),
                Mathf.MoveTowards(fadeImage.color.a, targetColor.a,
                    targetDurMod.w * Time.deltaTime));
        }
    }
}
